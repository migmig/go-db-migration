...
### 3. 백엔드: API/연계
- [ ] `POST /api/migrations/precheck` 구현
- [ ] pre-check 결과 필터 조회 API 구현
- [ ] 기존 마이그레이션 실행 API와 `use_precheck` 연계
- [ ] 입력 검증(정책/필터 enum) 및 에러 코드 표준화

### 4. 프론트엔드: pre-check UX
- [ ] pre-check 실행 버튼 및 요약 카드 추가
- [ ] 결과 테이블 컬럼(원본/대상/차이/판정) 추가
- [ ] decision 필터 탭 추가 (`all`, `transfer_required`, `skip_candidate`, `count_check_failed`)
- [ ] 실패 항목 reason 툴팁/경고 표시

### 5. CLI
- [ ] `--precheck-row-count` 플래그 추가
- [ ] `--precheck-policy` 플래그 추가
- [ ] `--precheck-filter` 플래그 추가
- [ ] dry-run 출력 포맷 확장

### 6. 관측성
- [ ] 구조화 로그 필드 추가 (`table_name`, `decision`, `policy`, `reason` 등)
- [ ] pre-check 관련 메트릭 추가
...
### 8. 운영 가이드/릴리즈
- [ ] README에 신규 플래그 및 pre-check 모드 설명 반영
- [ ] feature flag(`DBM_V19_PRECHECK`)로 점진 배포
